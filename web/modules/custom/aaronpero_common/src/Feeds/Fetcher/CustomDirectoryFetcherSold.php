 ...
 *
 * @FeedsFetcher(
 *   id = "custom_directory_sold",
 *   title = @Translation("Custom Directory Sold Listings"),
 ...
 * )
 */
class CustomDirectoryFetcherSold extends DirectoryFetcher {
  ...
  protected function preFetchParser() {
    $listings_dir = 'public://listings_feeds_sold/';

    \Drupal::service('file_system')->deleteRecursive($listings_dir);
    \Drupal::service('file_system')->mkdir($listings_dir);

    $parser = xml_parser_create();
    $string = file_get_contents('https://harcourts.co.nz/Listing/Rss?StaffID=32903&sold=true');
    ...
    for ($i = 1; $i <= $pages; $i++) {
      if ($i == 1) {
        ...
        $string = file_get_contents('https://harcourts.co.nz/Listing/Rss?StaffID=32903&sold=true&page=' . $i);
        $parser = xml_parser_create();
