 ...
 *
 * @FeedsParser(
 *   id = "custom_xml_single",
 *   title = @Translation("Custom XML Single"),
 *   description = @Translation("Parse XML with XPath.")
 * )
 */
class CustomXmlParserSingle extends XmlParser {
  ...
  protected function executeSources($row, array $expressions, array $variable_map) {
    ...
    foreach ($expressions as $machine_name => $expression) {
      ...
      elseif ($expression == 'ListingStatus') {
        $variables[$machine_name] = 'Sold';
        $item->set($machine_name, 'Sold');
      }
      elseif ($expression == 'CreationMethod') {
        $variables[$machine_name] = 'Single XML Import';
        $item->set($machine_name, 'Single XML Import');
      }
